...
5. Cloudflare Pages يبقى للاستضافة + Function خفيفة فقط لتسجيل الزيارات وكلمات البحث الفاشلة.
6. Netlify يبقى استضافة Static، مع تحويل التتبع وكلمات البحث الفاشلة إلى Cloudflare.
...
هذا ينشئ جداول الزيارات وكلمات البحث الفاشلة.

## ماذا يحدث يوميًا؟

- 20:30 UTC: GitHub Actions يحدث الترددات ويولد آخر التحديثات وملفات SEO ويعمل commit.
- 21:05 UTC: GitHub Actions يجلب زيارات اليوم السابق من Cloudflare D1 ويرسل تقرير Resend.

## ملاحظات مهمة

- Netlify لا يشغّل Functions في هذه النسخة؛ هو يستضيف الملفات فقط.
- Cloudflare يشغّل فقط وظائف خفيفة: `/api/track-visit` و `/api/search-feedback` ونقطة استقبال كلمات البحث الفاشلة بدون لوحة إدارة.
