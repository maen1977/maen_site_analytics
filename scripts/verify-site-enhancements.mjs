...
assert(pages.every((html) => html.includes("/assets/football-matches.js?v=20260826-matches-v8") && html.includes('id="sports"') && html.includes('id="sportsMatchesPanel"')), "Football schedule assets missing from a page");
...
assert(matchesWorkflow.includes("cron: '17 1,13 * * *'") && matchesWorkflow.includes("npm ci --no-audit --no-fund") && matchesWorkflow.includes("xvfb-run -a npm run github:update-matches") && matchesWorkflow.includes("football-broadcast-coverage.json") && matchesWorkflow.includes("test-football-matches.mjs") && matchesWorkflow.includes("newsCollection: false") && !matchesWorkflow.includes("update-sports-news") && !matchesWorkflow.includes("OPENAI_API_KEY"), "Matches-only twice-daily workflow is missing");
